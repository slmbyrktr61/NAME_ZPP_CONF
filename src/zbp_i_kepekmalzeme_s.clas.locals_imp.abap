CLASS LHC_KEPEKMALZEMETABLALL DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PUBLIC SECTION.
    CONSTANTS:
      CO_ENTITY TYPE abp_entity_name VALUE `ZI_KEPEKMALZEME_S`,
      CO_TRANSPORT_OBJECT TYPE mbc_cp_api=>indiv_transaction_obj_name VALUE `ZKEPEKMALZEMET`,
      CO_AUTHORIZATION_ENTITY TYPE abp_entity_name VALUE `ZI_KEPEKMALZEME`.
...
CLASS LSC_KEPEKMALZEMETABLALL IMPLEMENTATION.
  ...
  mbc_cp_api=>rap_bc_api( )->update_last_changed_date_time(
    maintenance_object = 'ZKEPEKMALZEME'
  ...
  ENDMETHOD.
ENDCLASS.
CLASS LHC_KEPEKMALZEME DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PUBLIC SECTION.
    CONSTANTS:
      CO_ENTITY TYPE abp_entity_name VALUE `ZI_KEPEKMALZEME`.

  PRIVATE SECTION.
    METHODS:
      GET_GLOBAL_FEATURES FOR GLOBAL FEATURES
        IMPORTING
          REQUEST REQUESTED_FEATURES FOR KepekMalzeme
        RESULT result.
ENDCLASS.

CLASS LHC_KEPEKMALZEME IMPLEMENTATION.
